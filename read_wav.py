...
#     print(hex(val[0]).zfill(6), end = '')
#     print(hex(val[1]).zfill(6))
# data = []
counter = 0
for i in range(1000):
    print(wavefile.readframes(1))
# with open(filename + ".txt", 'a') as fwrite:
#     for i in range(numframes):
#         val = wavefile.readframes(1)
#         # fwrite.write(str(hex(val[0])) + ',')
#         # fwrite.write(str(hex(val[1])) + ',')
#         tmp = ((val[1] << 8) + val[0]) // 16
#         fwrite.write(str(tmp) + ',')
#         counter += 1
#         if counter % 64 == 0:
#             fwrite.write('\n')
